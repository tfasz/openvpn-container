...
def test_notify_cli():
    with patch.object(slack_sdk.webhook.WebhookClient, 'send', return_value=MockResponse(200, "Ok")):
        result = CliRunner().invoke(notify, ["-D", test_dir, "-e", "client-connect", "-c", "test"])
        print(f"{result.output}")
        assert result.exit_code == 0
        assert result.output == "Slack notification status #200:Ok\n"

def test_notify_cli_disconnect():
    with patch.object(slack_sdk.webhook.WebhookClient, 'send', return_value=MockResponse(200, "Ok")):
        result = CliRunner().invoke(notify, ["-D", test_dir, "-e", "client-disconnect", "-c", "test", "-d", "60",
                                             "-s", "100", "-r", "200"])
        print(f"{result.output}")
        assert result.exit_code == 0
        assert result.output == "Slack notification status #200:Ok\n"
